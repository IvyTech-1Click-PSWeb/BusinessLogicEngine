...
{
    public class HTCTRIGGERENGINEERING : JGS.Web.TriggerProviders.TriggerProviderBase
    {
        ...
        public HTCTRIGGERENGINEERING()
        {
            this.Name = "HTCTRIGGERENGINEERING";
        }
        ...
        public override XmlDocument Execute(System.Xml.XmlDocument xmlIn)
        {
            ...
            if (TriggerType == "TIMEOUT")
            {
                ...
                if (resultCode.ToUpper() == "PURGE")
                {
                    Boolean bfaillist = false;
                    XmlNodeList xnList = xmlIn.SelectNodes("/Trigger/Detail/TimeOut/WCFlexFields/FlexField");
                    foreach (XmlNode xn in xnList)
                    {
                        if (xn["Name"].InnerText=="Pass_Purge")
                            if (xn["Value"].InnerText != String.Empty & xn["Value"].InnerText == "Purge2010")
                            {
                 ...
                 if (bfaillist == false)
                  {
                        return SetXmlError(returnXml, "The Result Code or Code is Bad|El Result Code o el codigo es incorrecto");
                  }
                }
