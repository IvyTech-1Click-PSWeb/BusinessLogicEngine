...
{
    public class HTCTRIGGERDIAGNOSIS : JGS.Web.TriggerProviders.TriggerProviderBase
    {
        ...
        public HTCTRIGGERDIAGNOSIS()
        {
            this.Name = "HTCTRIGGERDIAGNOSIS";
        }
        ...
        public override XmlDocument Execute(System.Xml.XmlDocument xmlIn)
        {
            ...
            {
                if ((PartNo == "99HKY001-00") || (PartNo == "99HKY001-01") || (PartNo == "99HKY002-00") || (PartNo == "99HKY001-00_BBY") || (PartNo == "99HKY001-01_BBY") || (PartNo == "99HKY002-00_BBY"))
                    {
                     List<OracleParameter> myParams4;
                     myParams4 = new List<OracleParameter>();
                     myParams4.Add(new OracleParameter("ClientId", OracleDbType.Int32, clientId.ToString().Length, ParameterDirection.Input) { Value = clientId });
                     myParams4.Add(new OracleParameter("ContractId", OracleDbType.Int32, contractId.ToString().Length, ParameterDirection.Input) { Value = contractId });
                     myParams4.Add(new OracleParameter("LocationID", OracleDbType.Int32, LocationID.ToString().Length, ParameterDirection.Input) { Value = LocationID });
                     myParams4.Add(new OracleParameter("ITEMID", OracleDbType.Int32, ItemID.ToString().Length, ParameterDirection.Input) { Value = ItemID });
                     myParams4.Add(new OracleParameter("UserName", OracleDbType.Varchar2, UserName.Length, ParameterDirection.Input) { Value = UserName });
                     getFailureReasonIDValue = Functions.DbFetch(this.ConnectionString, "WEBAPP1", "JGSHTCDIAGNOSIS", "GetFailureReasonID", myParams4);
                     if ((getFailureReasonIDValue != null) && (getFailureReasonIDValue.Length != 0))
                     {
                         if ((getFailureReasonIDValue == "15") || (getFailureReasonIDValue == "34") || (getFailureReasonIDValue == "36") || (getFailureReasonIDValue == "39"))
                         //.IndexOf("15,34,36,39")
                         {
                             if (RC.ToUpper() != "NEOCORE")
                             {
                                 Msg = "La unidad debe ser direccionada a NeoCore";
                         ...
                         else
                         {
                             if (RC.ToUpper() == "NEOCORE")
                             {
                                 Msg = "Este Result Code no es válido";
                                 return SetXmlError(returnXml, Msg);
                             }
                         }
                     }
                     ...
                     //if (RC.ToUpper() == "PASS")
                     //{
                     //    Msg = "Este Result Code no es valido para este Modelo";
                     //    return SetXmlError(returnXml, Msg);
                    // }
                    //return SetXmlError(returnXml, Msg);
            }
