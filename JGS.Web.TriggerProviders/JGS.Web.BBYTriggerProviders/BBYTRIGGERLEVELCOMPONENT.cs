...
{
    public class BBYTRIGGERLEVELCOMPONENT : JGS.Web.TriggerProviders.TriggerProviderBase
    {
        ...
        public BBYTRIGGERLEVELCOMPONENT()
        {
            this.Name = "BBYTRIGGERLEVELCOMPONENT";
        }
        ...
        public override XmlDocument Execute(System.Xml.XmlDocument xmlIn)
        {
            ...
            if (Trigger.ToUpper() == "TIMEOUT")
            {
                    ...
                    Nivel = Functions.DbFetch(this.ConnectionString, "WEBAPP1", "JGSBBYLEVELCOMPONENT", "GETLEVEL", myParams);
                    if (Nivel == "OK")
                    {
                        RC = "NFF_RP";
                    }
                    ...
                    Functions.UpdateXml(ref returnXml, _xPaths["XML_RESULTCODE"], RC);
                    SetXmlResultCode(returnXml, RC);
                    List<OracleParameter> myParams2;
                    myParams2 = new List<OracleParameter>();
                    myParams2.Add(new OracleParameter("BCN", OracleDbType.Varchar2, BCN.ToString().Length, ParameterDirection.Input) { Value = BCN });
                    myParams2.Add(new OracleParameter("UserName", OracleDbType.Varchar2, UserName.Length, ParameterDirection.Input) { Value = UserName });
                    Result = Functions.DbFetch(this.ConnectionString, "WEBAPP1", "JGSBBYLEVELCOMPONENT", "GETRCANT", myParams2);
                    if  (RC.ToUpper()=="RNR")
                    {
                        if (Result == "OK")
                        {
                                Msg = "Trigger Error: No se puede seleccionar este result code";
        ...
        /// <param name="returnXml"></param>
        /// <returns></returns>
        private void SetXmlResultCode(XmlDocument returnXml, string RC )
        {
            Functions.UpdateXml(ref returnXml, _xPaths["XML_RESULTCODE"], RC);
        }
