...
namespace JGS.BusinessLogicEngine.API
{
    public class RemovePart : IAPI
    {
        ...
        {
            FailureAnalysisWrapper.RemovePartsInfo info = new FailureAnalysisWrapper.RemovePartsInfo();
            ...
            info.removeNonInventoryParts = new List<FailureAnalysisWrapper.IssueRemoveNonInventoryPartDO>();
            foreach (XmlNode dcNode in document.SelectNodes(fields.Where(prop => prop.Name == "FA_DEFECT_CODE").First().XPath))
            {
                ...
                {
                    foreach (XmlNode npNode in acNode.SelectNodes(fields.Where(prop => prop.Name == "FA_OLD_COMPONENT").First().XPath))
                    {
                        ...
                        info.removeNonInventoryParts.Add(issue);
            ...
            if (info.removeNonInventoryParts.Count == 0)
            {
                info.removeNonInventoryParts = null;
            }
            ...
            _client.CheckClient();
            string result = _client.PerformRemoveParts(info, false);
